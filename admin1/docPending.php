		...
		<div class="container-fluid">
			...
			<ol class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="docPending.php?adminID=<?php echo($adminID)?>">Request</a>
				</li>
				...
				<table id="dataTable" class="table tbl table-striped table-bordered" width="100%" style="text-align: center;">
					...
					<tbody>
							...
							if (isset($_POST['findType'])) {
								...
								if($row1>0){
									while($row2=mysqli_fetch_array($query)){ 
										$type = $row2['type'];
							            $amount = $row2['amount'];	
										$qry=mysqli_query($con, "SELECT * FROM request WHERE docID = '$row2[docID]' AND status = 'PENDING'");
				    						...
				    						<td>
				    							<?php
				    								if ($type == 'Certificate of Indigency') {
				    									echo "<button type='button' class='btn btn-primary btn-xs' style='margin-right:16px;'>
							    								<a style='color: white' target='_Blank' href='indigency.php?requestID=$fetch[requestID]' data-toggle='tooltip' data-placement='bottom' title='View Request'><i class='fa fa-print'></i></a>
															</button>";
				    								}else if ($type == "Certificate of Residency") {
				    									echo "<button type='button' class='btn btn-primary btn-xs' style='margin-right:16px;'>
							    								<a style='color: white' target='_Blank' href='residency.php?requestID=$fetch[requestID]' data-toggle='tooltip' data-placement='bottom' title='View Request'><i class='fa fa-print'></i></a>
															</button>";
				    								}else if ($type == "Barangay Clearance") {
				    									echo "<button type='button' class='btn btn-primary btn-xs' style='margin-right:16px;'>
							    								<a style='color: white' target='_Blank' href='clearance.php?requestID=$fetch[requestID]' data-toggle='tooltip' data-placement='bottom' title='View Request'><i class='fa fa-print'></i></a>
															</button>";
				    								}else if ($type == "Recommendation") {
				    									echo "<button type='button' class='btn btn-primary btn-xs' style='margin-right:16px;'>
							    								<a style='color: white' target='_Blank' href='recommendation.php?requestID=$fetch[requestID]' data-toggle='tooltip' data-placement='bottom' title='View Request'><i class='fa fa-print'></i></a>
															</button>";
				    								}
				    							?>
				    						</td>
						...
						<?php
								...
								$query=mysqli_query($con, "SELECT * FROM `request` WHERE status = 'PENDING'") or die(mysqli_error());
								while($fetch=mysqli_fetch_array($query)){
									$qry=mysqli_query($con, "SELECT * FROM document WHERE docID = '$fetch[docID]'");
					                	while ($row2=mysqli_fetch_assoc($qry)) {
					                		$type = $row2['type'];
