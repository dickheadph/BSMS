...
<body class="fixed-nav sticky-footer" id="page-top">
		...
		<div class="collapse navbar-collapse" id="navbarResponsive">
			<ul class="navbar-nav navbar-sidenav" id="exampleAccordion" style="background-color: #c2523e;">
				...
				<li class="nav-item" data-toggle="tooltip" data-placement="right" title="Archive">
					<a class="nav-link active" href="archive.php?adminID=<?php echo ($adminID)?>" style="color: white;">
	...
	<?php
		...
		$qr = mysqli_query($con, "SELECT * FROM `payment`");
		while($r = mysqli_fetch_array($qr)){
			if(date('Y-m-d', strtotime($r['pDateTime'])) < date('Y-m-d', strtotime('-5 years'))){
				$date = date('Y-m-d', strtotime($r['pDateTime']));
				$time = date('H:i:s', strtotime($r['pDateTime']));
				mysqli_query($con, "INSERT INTO `archive` VALUES('', 2, '$r[type]', NULL, '$date', '$time', '$r[receipt]', NULL, NULL, '$r[amount]', '$r[orNum]', '$r[resID]', NULL)") or die(mysqli_error($con));
				mysqli_query($con, "DELETE FROM `payment` WHERE `payID` = '$r[payID]'") or die(mysqli_error($con));
			}
		}
		...
		<div class="container-fluid">
			...
			<ol class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="archive.php?adminID=<?php echo($adminID)?>">Archive</a>
				</li>
					...
					<tbody>
						<?php
							if(isset($_POST['search'])){
								$date1 = $_POST['date1'];
								$date2 = $_POST['date2'];
								$query=mysqli_query($con, "SELECT * FROM `archive` WHERE `date` BETWEEN '$date1' AND '$date2'") or die(mysqli_error());
									...
									<div class="modal-content">
										<form method="POST" action="" enctype="multipart/form-data">
											<div class="modal-header">
												<h3 class="modal-title" style="color: white">Payment Details</h3>
											</div>
											<div class="modal-body" style="color: white">
												<div class="row">
													<input type="hidden" name="arcID" value="<?php echo $fetch['arcID']?>">
													...
													<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
														...
														<i class="fa fa-money"></i> <label class="control-label">Receipt</label> <br>
														<img src="../receipt/<?php echo($fetch['image'])?>">
													</div>
												</div>
						...
						<?php
								...
								$query=mysqli_query($con, "SELECT * FROM `archive` WHERE docBlotter = 2 ORDER BY date DESC") or die(mysqli_error());
								$row2=mysqli_num_rows($query);
								if($row2>0){
									while($fetch=mysqli_fetch_array($query)){
										@$total += $fetch['amount'];
