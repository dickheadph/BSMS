...
<body class="fixed-nav sticky-footer" id="page-top">
	...
	<div class="content-wrapper">
		<div class="container-fluid">
			...
			<button type="button" class="btn edit" style="margin-bottom: 10px; margin-right: 3px; float: right" data-toggle="modal" data-target="#add_data_Modal">
    			<i class="fa fa-plus-circle"></i> Add Residents
    		</button>
			<form method="post" action="" class="form-inline" style="margin-bottom: 5px; float: left;">
				...
				<!-- <span><a href="list_print.php" target="_new" class="btn edit"><i class="fa fa-print"></i></a></span> -->
			</form>
			
			<div class="table-responsive">
				<table id="dataTable" class="table tbl table-bordered" width="100%" style="text-align: center;">
					<thead>
						<tr>
					...
					<tbody>
						<?php
								...
								if($row2>0){
									while($fetch=mysqli_fetch_array($query)){ ?>
										<tr>
											...
											<td>
												<button type="button" class="btn btn-primary btn-xs dt-edit" data-placement="bottom" title="Edit" data-toggle="modal" data-target="#update_modal<?php echo $fetch['resID'] ?>">
									    			<i class="fa fa-fw fa-edit"></i>
									    		</button>
											</td>
										</tr>
									<?php
										include 'ud_modal.php';
									}
								}else{
									echo'<tr>
							...
							else if (isset($_POST['findZone'])) {
								$zone = $_POST['zone'];

								$query=mysqli_query($con, "SELECT * FROM resident WHERE resZone = '$zone'") or die(mysqli_error());
								$row2=mysqli_num_rows($query);
								if($row2>0){
									while($fetch=mysqli_fetch_array($query)){ ?>
										<tr>
						...
						?>
							<tr>
	    						...
	    						<td><?php echo strtoupper($fetch['resZone'])?></td>
	    						<td>
	    							<button type="button" class="btn btn-primary" data-placement="bottom" title="Edit" data-toggle="modal" data-target="#update_modal<?php echo $fetch['resID'] ?>">
						    			<i class="fa fa-fw fa-edit"></i>
						    		</button>
	    						</td>
							</tr>
