		...
		<div class="container-fluid">
			...
			<ol class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="docComplete.php?adminID=<?php echo($adminID)?>">Request</a>
				</li>
				<li class="breadcrumb-item active">Completed Document</li>
			</ol>
				...
				<table id="dataTable" class="table tbl table-striped table-bordered" width="100%" style="text-align: center;">
					...
					<tbody>
							...
							if (isset($_POST['findType'])) {
								...
								if($row1>0){
									while($row2=mysqli_fetch_array($query)){ 
										$type = $row2['type'];
							            $amount = $row2['amount'];	
										$qry=mysqli_query($con, "SELECT * FROM request WHERE docID = '$row2[docID]' AND status = 'COMPLETED'");
						...
						<?php
								...
								$query=mysqli_query($con, "SELECT * FROM `request` WHERE status = 'COMPLETED'") or die(mysqli_error());
								while($fetch=mysqli_fetch_array($query)){
									$qry=mysqli_query($con, "SELECT * FROM document WHERE docID = '$fetch[docID]'");
					                	while ($row2=mysqli_fetch_assoc($qry)) {
					                		$type = $row2['type'];
