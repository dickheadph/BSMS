				...
				<li class="nav-item" data-toggle="tooltip" data-placement="right" title="Requests">
					...
					<ul class="sidenav-second-level collapse" id="collapseExamplePages" style="background-color: #c2523e;">
						<li class="nav-item" data-toggle="tooltip" data-placement="right" title="Documents">
							<a class="nav-link active nav-link-collapse collapsed" data-toggle="collapse" href="#collapseExamplePagess" data-parent="#exampleAccordion" style="color: white;">
								<i class="fa fa-fw fa-file"></i>
		...
		<div class="container-fluid">
			...
			<ol class="breadcrumb">
				<li class="breadcrumb-item">
					<a href="docPaid.php?adminID=<?php echo($adminID)?>">Request</a>
				</li>
				<li class="breadcrumb-item active">Paid Document</li>
			</ol>
					...
					<tbody>
						<?php
							if (isset($_POST['findType'])) {
								...
								$query=mysqli_query($con, "SELECT * FROM document WHERE type = '$type'") or die(mysqli_error());
								...
								if($row1>0){
									while($row2=mysqli_fetch_array($query)){ 
										...
										$qry=mysqli_query($con, "SELECT * FROM request WHERE docID = '$row2[docID]' AND status = 'PAID'");
					                	while ($fetch=mysqli_fetch_assoc($qry)) {
						?>
										<tr>
											<td><?php echo $fetch['requestID'] ?></td>
				    						<td><?php echo $type ?></td>
				    						<td><?php echo $fetch['purpose']?></td>
				    						<td><?php echo date("M d, Y", strtotime ($fetch['dDate']))?></td>
				    						...
				    						<td>
				    							<button type='button' class='btn btn-success btn-xs' style='margin-right:16px;'>
				    								<a style='color: white' href='#' data-toggle='modal' data-target='#doneModal<?php echo $fetch['requestID']?>' data-placement='bottom' title='Done'><i class='fa fa-check'></i></a>
												</button>
				    						</td>
										</tr>
										<!-- Completed Modal -->
										<div class="modal fade" id="doneModal<?php echo $fetch['requestID']?>" aria-hidden="true">
											<div class="modal-dialog" role="document">
												<div class="modal-content">
													<form method="POST" action="completed.php">
														<div class="modal-header">
															<h5 class="modal-title" style="color: white">Completed Document?</h5> <br>
															<button type="submit" name="done" class="btn" style="background-color: #f8a698"><span class="fa fa-check"></span> Yes</button>
															<button type="button" data-dismiss="modal" class="btn" style="background-color: #f8a698"><span class="fa fa-warning"></span> No</button>
														</div>
														<input type="hidden" name="requestID" value="<?php echo $fetch['requestID']?>">
						...
						<?php
								...
								$date1 = $_POST['date1'];
								$date2 = $_POST['date2'];
								$query=mysqli_query($con, "SELECT * FROM `request` WHERE `dDate` BETWEEN '$date1' AND '$date2'") or die(mysqli_error());
								$row2=mysqli_num_rows($query);
								if($row2>0){
									while($fetch=mysqli_fetch_array($query)){
										$qry=mysqli_query($con, "SELECT * FROM document WHERE docID = '$fetch[docID]'");
					                	while ($row2=mysqli_fetch_assoc($qry)) {
					                		$type = $row2['type'];
							            	$amount = $row2['amount'];

							            	$qq=mysqli_query($con, "SELECT * FROM resident WHERE resID = '$fetch[resID]'");
						                	while ($row3=mysqli_fetch_assoc($qq)) {
						                		$resNum = $row3['resNum'];
						                		$fullName = ucwords($row3['resFName'])." ".ucwords($row3['resLName']);
						?>
							<tr>
								<td><?php echo $fetch['requestID'] ?></td>
	    						<td><?php echo $type ?></td>
						...
						<?php
								...
								$query=mysqli_query($con, "SELECT * FROM `request` WHERE status = 'PAID'") or die(mysqli_error());
								while($fetch=mysqli_fetch_array($query)){
									$qry=mysqli_query($con, "SELECT * FROM document WHERE docID = '$fetch[docID]'");
					                	while ($row2=mysqli_fetch_assoc($qry)) {
					                		$type = $row2['type'];
